...
function AppShell(){
  ...
  // Build a lookup: species_slug → array of uploaded image URLs (prefer thumb_url).
  const uploadsBySlug = useMemo(() => {
  const out = {};
  for (const c of (claims || [])) {
    if (!c.species_slug) continue;
    const url = c.thumb_url || c.photo_url;
    if (!url) continue;
    if (!out[c.species_slug]) out[c.species_slug] = [];
    out[c.species_slug].push(url);
  }
  return out;
}, [claims]);

// 50/50 image chooser
function randomFishImage(defaultImg, slug) {
  const list = uploadsBySlug[slug] || [];
  if (list.length === 0) return defaultImg;
  const coin = Math.random() < 0.5;
  if (!coin) return defaultImg;
      ...
      <Route
        ...
        element={
          <PlayPage
            ...
            pMap={pMap}
            compCfg={compCfg}
			randomFishImage={randomFishImage}
      uploadsBySlug={uploadsBySlug}
          />
        }
      />
...
function PlayPage(props){
  const {
    ...
    reloadAll, reloadMine, pMap, compCfg,
	 randomFishImage, uploadsBySlug     // ⭐ NEW
  } = props
            ...
            return (
              <div key={s.slug} className='card'>
                <div className='img-box'>
                 <img
  src={randomFishImage(imgFor(s), s.slug)}
  alt={s.name}
  onError={(e)=>{ 
